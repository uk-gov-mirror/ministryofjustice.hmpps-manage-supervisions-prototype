...
class ArrangedSession {
  constructor(providerCode, typeOfSession, date, startTime, endTime, countsTowardsRAR, rarCategory) {
  ...
  get summary() {
    return {
      'typeOfSession': this.typeOfSession || 'Office visit',
      'date': this.date,
...
class RARCategories {
  constructor(providerCode, typeOfSession) {
    this.providerCode = providerCode
    this.typeOfSession = typeOfSession
  }

  get asList() {
    return this.availableNSIsForSession.filter(value => Object.keys(this.availableNSIsForProvider).includes(value));
  }

  get descriptions() {
    return this.asList.map(code => this.availableNSIsForProvider[code].description).sort()
  }
...
exports.arrangedSession = (providerCode, typeOfSession, date, startTime, endTime, countsTowardsRAR, rarCategory) => {
  return new ArrangedSession(providerCode, typeOfSession, date, startTime, endTime, countsTowardsRAR, rarCategory)
}

exports.possibleRARCategories = (typeOfSession, providerCode) => {
  return new RARCategories(providerCode, typeOfSession).descriptions
}
